...
serverAddress = "https://localhost:8080/jenkins"
jobPipelineName = "job_name"
server = jenkins.Jenkins('%s' % serverAddress)

def server_name():
        print "JENKINS SERVER ADDRESS : ", serverAddress

def job_name():
        print "PIPELINE JOB NAME : ", jobPipelineName

def job_count():
        print "SERVER JOB COUNT: ", server.jobs_count()
...
def main():
        print("{You are at main def}")

        server_name()
        job_count()
        job_name()
        last_build_job_number()
        last_build_job_name()
        last_build_job_result()
        last_successful_build_job_number()
        last_successful_build_job_name()
        last_successful_build_job_result()
        last_unsuccessful_build_job_number()
        last_unsuccessful_build_job_name()
        last_unsuccessful_build_job_result()
        last_failed_build_job_number()
        last_failed_build_job_result()
        last_failed_build_job_result()
        job_health_report_score()
        job_health_report_description()
